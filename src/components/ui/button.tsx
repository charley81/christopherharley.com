...
const buttonVariants = cva(
  'inline-flex items-center justify-center gap-2 whitespace-nowrap text-sm font-medium transition-colors focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        default:
          'bg-surface-container-high text-text-primary hover:bg-surface-dim transition-colors duration-300',
        destructive: 'bg-error text-on-error hover:bg-error/90',
        outline:
          'border border-border-muted bg-background hover:bg-surface-container',
        secondary:
      ...
      size: {
        default:
          'py-4 px-8 font-label-mono text-label-mono uppercase tracking-widest',
        sm: 'h-9 px-3',
        lg: 'h-11 px-8',
        icon: 'h-10 w-10',
      },
